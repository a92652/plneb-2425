...
@app.route('/conceitos/<designacao>') # por defeito o route é get
...
def adicionar_conceito_api(): 
    ...
    # para não perder a informação quando o servidor vai abaixo - guardar num ficheiro
...
def delete_conceito(designacao):
    if designacao in db:
        ...
        del db[designacao]
        json.dump(db, f_out, indent = 4, encoding = "utf-8", ensure_ascii = False)
        f_out.close()
